csv2hpp
=======
This command-line tool converts databases in CSV file format (.CSV suffix) into header files to be included in C++ projects. This allows to immediately work on large, structured, well-formed datasets without the need of loading, allocating, checking, converting (the compiler is doing it). Please perform the following steps for conversion:
...
Hints
-----
* Datatype mapping is: (string) -> 'const char*', (int) -> 'int', (long) -> 'long', (float) -> 'float', (double) -> 'double', (bool) -> 'bool', (_km) -> 'SI:length' and so on.
