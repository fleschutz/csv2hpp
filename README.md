...
💡 Hints
--------
* The mapping of datatype hints to C/C++ datatypes is: `(int)` → `int`, `(float)` → `float`, `(string)` → `const char*`, `(skip)` skips the entire column. All supported datatype hints are listed in: [datatype_hints.csv](src/datatype_hints.csv)
...
🎉 Example
-----------
Using [planets.csv](examples/planets.csv) and executing: `csv2hpp planets.csv planet > planets.hpp` results in the header file: [planets.hpp](examples/planets.hpp).
